...
class DataStorage {
  ...
  private calculateSummaryAndCategories(transactions: Transaction[]): {
    ...
    transactions.forEach(transaction => {
      ...
      const categoryData = categories[transaction.mainCategory][transaction.subCategory];
  ...
  public saveMonthlyData(monthlyData: MonthlyData): void {
    ...
    const { summary, categories } = this.calculateSummaryAndCategories(monthlyData.transactions);
    ...
    try {
      ...
      console.error(`Error saving monthly data for ${monthlyData.year}-${monthlyData.month}:`, error);
      throw error;
    }
  }
  ...
  public getRecentTransactions(limit: number = 10): Transaction[] {
    ...
    for (let monthOffset = 0; monthOffset < 12 && allTransactions.length < limit; monthOffset++) {
      const targetDate = new Date(currentYear, currentMonth - 1 - monthOffset, 1);
      const targetYear = targetDate.getFullYear();
      const targetMonth = targetDate.getMonth() + 1;
      
      const monthTransactions = this.getMonthlyTransactions(targetYear, targetMonth);
  ...
  // Get monthly summary
  public getMonthlySummary(year: number, month: number): MonthlyData['summary'] {
    const monthlyData = this.loadMonthlyData(year, month);
  ...
  public getMonthlyCategoryBreakdown(year: number, month: number): MonthlyData['categories'] {
    const monthlyData = this.loadMonthlyData(year, month);
  ...
  public getYearlySummary(year: number) {
    const summary = {
      ...
      balance: 0,
      transactionCount: 0,
      monthlyBreakdown: [] as Array<{ month: number; summary: MonthlyData['summary'] }>,
    };
  ...
  public getAvailableYears(): number[] {
    try {
      const years = fs.readdirSync(this.dataDir)
        .filter(item => {
